...
for (let i = 0; i < data.results.forecast.length; i++) {
   console.log(`Today is ${data.results.forecast[i].day.day}, ${data.results.forecast[i].day.date}. The high for today is ${data.results.forecast[i].day.high}, and the low will be ${data.results.forecast[i].day.low}. It will be ${data.results.forecast[i].day.text}.`)   
}
...
for (let i = 0; i < myJSON.people.length; i++) {
   console.log(`Full Name: ${obj.employees[i].firstName} ${obj.employees[i].lastName} || Job: ${myJSON.people[i].job}`);
}